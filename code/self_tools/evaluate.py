...
def evaluate_for_train(hidden_dim, idx_train, idx_val, idx_test, label, nb_classes, device, dataset, lr, wd,
    ...
    test_lbls = torch.argmax(label[idx_test], dim=-1).cpu().numpy()
    test_emb = emb[idx_test].cpu().numpy()
    # print(len(test_lbls))
    # print(test_emb.shape)

    nmis, aris = [], []

    # Run K-means 10 times (to account for randomness)
    for seed in range(10):
        kmeans = KMeans(n_clusters=nb_classes, random_state=42)
        cluster_labels = kmeans.fit_predict(test_emb)

        # Compute NMI and ARI
        nmi = normalized_mutual_info_score(test_lbls, cluster_labels)
        ari = adjusted_rand_score(test_lbls, cluster_labels)

        nmis.append(nmi)
        aris.append(ari)

    # Find the best run (highest NMI)
    best_run = np.argmax(nmis)
    best_nmi = nmis[best_run]
    best_ari = aris[best_run]

    print(
        f"[Clustering] "
        f"Best NMI: {best_nmi:.4f}, "
        f"Best ARI: {best_ari:.4f}"
    )
